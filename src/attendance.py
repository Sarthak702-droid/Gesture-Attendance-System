...
def sync_to_excel():
    ...
    try:
        ...
        with pd.ExcelWriter(XLSX_PATH, engine="openpyxl") as writer:
            ...
            # Format header cells (make them bold and add dark blue background)
...
def mark_attendance(name, status, location, evidence_path, voice_excuse_path=""):
    ...
    day_str = now.strftime("%A")  # Full day name (e.g. Monday, Tuesday)
        ...
        try:
            with open(CSV_PATH, "r", newline="", encoding="utf-8") as f:
                ...
                for row in reversed(rows[1:]):  # Skip header row
                    if len(row) >= 8:
                        row_name, row_status, _, _, _, _, _, row_ts_str = row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7]
                        try:
...
def git_push_logs_async():
    ...
    def push():
        try:
            import subprocess
            # Stage files
            subprocess.run(["git", "add", "data/attendance.csv", "data/attendance.xlsx", "data/broadcasts.json", "data/security_logs.json", "data/alerts/", "data/config.json"], capture_output=True)
